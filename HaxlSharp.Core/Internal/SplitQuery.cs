...
namespace HaxlSharp.Internal
{
    ...
    {
        public static Haxl ToFetch(BindSegments split, string parentBind, Scope parentScope)
        {
            if (parentScope == null) parentScope = Scope.New();
            var rebinder = new RebindToScope();
            Haxl finalFetch = null;
            Action<Func<Scope, Haxl>> bind = f =>
            {
            ...
            foreach (var segment in split.Segments)
            {
                ...
                Func<Scope, Haxl> currentGroup = scope =>
                    segment.BoundExpressions.Aggregate(Haxl.FromFunc(c => Done.New(s => s)), (group, be) =>
                    {
                        var rewritten = rebinder.Rebind(be.Expression);
                        dynamic wrapped = rewritten.Compile().DynamicInvoke(scope);
                        var fetch = (Haxl)wrapped.ToHaxlFetch(be.BindVariable, scope);
